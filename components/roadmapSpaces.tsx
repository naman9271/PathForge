        ...
        <div className="flex flex-col gap-3">
                ...
                roadmaps && roadmaps.length > 0 && (
                    roadmaps.map((roadmap: any) => (
                        roadmap._id === roadmapId ? (
                            <div key={roadmap._id}
                                className="w-full py-5 bg-white rounded-lg text-black p-2 cursor-pointer  flex flex-row gap-1 justify-between"
                            ...
                            <div key={roadmap._id}
                                className="w-full py-5 bg-white rounded-lg text-black p-2 cursor-pointer"
                                onClick={() => { handleRoadmap(roadmap._id); }}
                            >
                                {roadmap.name}
                            </div>
            ...
            }
            {
                roadmaps && roadmaps.length < 3 && (
                    <button className="flex justify-center w-full py-5 bg-white rounded-lg text-black text-center" title="Create Roadmap Space" onClick={createRoadmapSpace}>
                        <FaPlus />
                    </button>
                )
            }
